   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (defun mymail(days)
       ...
       (let ((days (if  days  (prefix-numeric-value days) 3 ))
             (header (get-buffer "*mu4e-headers*")))
         (if (and header (> 4 days))
             (switch-to-buffer header)
           (mu4e-headers-search (format  "date:%dd..now" days)))))




     (global-set-key [f12] 'mymail)
     (global-set-key [S-f12] 'mu)
     (global-set-key [M-f12] '(lambda ()  (interactive) (pop-to-buffer "*mu4e-headers*")))
