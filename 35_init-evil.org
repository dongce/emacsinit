...
* switch-window
  ...
  :config
  (global-set-key (kbd "<f12>") 'switch-to-buffer)
...
* garbage-collection
  ...
  #+RESULTS:
  | lambda | nil | (interactive) | (save-some-buffers t #'(lambda nil (and (not buffer-read-only) (buffer-file-name)))) | (garbage-collect) |

* evil-states
** normal state
   #+BEGIN_SRC emacs-lisp
   (append-to-list 'evil-normal-state-modes '(mu4e-compose-mode occur-edit-mode))
   #+END_SRC
