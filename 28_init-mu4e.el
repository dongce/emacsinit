...
(defun open-mu4e-header ()
  ...
  (w32-shell-execute
   nil
   (mu4e-filename (mu4e-message-at-point) )
   nil
   1)
  )
...
(defun open-mu4e-view ()
  (interactive)
  (w32-shell-execute nil (mu4e-filename mu4e~view-msg ) nil 1))
  ...
  (let* ((embedded ;; is it registered as an embedded msg (ie. message/rfc822
    ...
    (with-current-buffer buf
      (let ((inhibit-read-only t))
        ...
        (save-excursion          ;;!!!ticket:XXXX 20121130 김동일 | HTML RENDERING
          (goto-char (point-min));;!!!ticket:XXXX 20121130 김동일 | HTML RENDERING
          (forward-paragraph)    ;;!!!ticket:XXXX 20121130 김동일 | HTML RENDERING
          (htmlr-render)
          ;; (shr-render)
          )        ;;!!!ticket:XXXX 20121130 김동일 | HTML RENDERING
