...
** setting
   #+BEGIN_SRC emacs-lisp
     ...
     (setq org-default-notes-file "u:/orgdir/notes.org")
...
** pygmentize 
   ...
   (defvar pygmentize (w32-unix-eval ( "t:\\usr\\local\\python35\\Scripts\\pygmentize.exe" ) ("pygmentize") ))
...
** calendar
   #+BEGIN_SRC emacs-lisp
     (setq
      ...
      org-caldav-inbox "u:/orgdir/caldav.org"
      )
   #+END_SRC
* evil conflict
#+BEGIN_SRC emacs-lisp
  ...
  the links being already placed inside the text."
    ...
    (let* ((scrfilename (concat (file-name-as-directory
      ...
      (if (and (file-exists-p scrfilename)
        ...
        (let* ((scrpath (s-replace "/" "\\" (convert-standard-filename  (expand-file-name scrfilename))))
