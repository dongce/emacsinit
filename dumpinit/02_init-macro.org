  ...
  (let*
      ((group
        (selectrum-completing-read
         "그룹을 선택하세요 "
         (-map (lambda (x) (cdr  (assoc 'GroupName x )))
               (with-temp-buffer
                 (url-insert-file-contents "http://10.239.23.107:8880/Navigation?Command=list_group" )
        ...
        (append
         (with-temp-buffer
           (url-insert-file-contents
            (format  "http://10.239.23.107:8880/Navigation?Command=list_url&GroupName=%s" (symbol-name 'BUILDCENTER)) )
           (json-parse-buffer :object-type 'alist)) nil)
        )
       (urlname
        (selectrum-completing-read
         " 주소를 선택하세요 "
         (-map (lambda (x) (cdr  (assoc 'UrlName x ))) urls))
        )
       (url
        (cdr
         (assoc 'Url
...
* monitor change
  #+begin_src emacs-lisp
    ...
    (defun mymonitor-helper (mon )
      (powershell
       (concat
        "t:/usr/local/controlmymonitor/ControlMyMonitor.exe /setvalue \\\\.\\DISPLAY2\\Monitor0 60 "
        (format "%d" mon))))
    
    (defun* mymonitor (&optional (mon 1))
      (interactive "p")
      (mymonitor-helper mon))
  #+end_src
